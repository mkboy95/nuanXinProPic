...
# 已知的一级分类（用于匹配）
KNOWN_L1_CATEGORIES="动漫|游戏|影视|风景|人像|插画|IP形象|萌宠|政治"

# 排除列表（这些文件已经存在但名字不同，跳过同步）
# 格式：Gitee文件名的核心部分（_后面的部分）
EXCLUDE_CORE_NAMES="白色翅膀|蘑菇人对战|彩虹色旋律的少女|花海中的优雅公主|持剑的白发红衣少女|沉思的白色蕾丝公主|仰望星空的城市夜景少女|夕阳下的红发少女与日本刀|黑衣长发动漫少女|绿意森林中的神秘少女"

# 获取L2分类的函数
get_l2_categories() {
    local l1="$1"
    case "$l1" in
        "动漫") echo "二次元|仙逆|刀剑神域|初音未来|剑来|名侦探柯南|哆啦A梦|喜洋洋与灰太狼|完美世界|小埋|斗破苍穹|新世纪福音战士|春物雪乃|猫和老鼠|百炼成神|神奇宝贝|紫罗兰永恒花园|罪恶王冠|蕾姆|蜡笔小新|进击的巨人|间谍过家家|鬼灭之刃" ;;
        "游戏") echo "原神|崩坏|艾尔登法环|英雄联盟|通用" ;;
        "影视") echo "疯狂动物城|海绵宝宝" ;;
        "风景") echo "雪山|海滨|城市|天空|日落|湖泊|花卉|建筑|星空|通用" ;;
        "人像") echo "氛围感|清新|魅力|国风|明星|张凌赫" ;;
        "插画") echo "二次元|国风|创意|场景|文字|卡通|通用" ;;
        "IP形象") echo "水豚噜噜|线条小狗|乌萨奇|凯蒂猫|粉红兔|通用" ;;
        "萌宠") echo "兔兔|狗狗|猫咪" ;;
        "政治") echo "通用" ;;
        *) echo "" ;;
    esac
